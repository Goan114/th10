...
export async function createNativeGame(module,{files,language='jp',fontBackend,onWrite,canvas,audioPort,onContext=()=>{},now,monotonic}={}){
 ...
 const archive=language==='chs'?'th10c.dat':'th10.dat';native.graphics.putText(name,archive);if(!c.files_attach(fileSystem,name))throw new Error('无法打开风神录资源包');
 ...
 const app=c.application_create(fileSystem,input,state,engine,fonts,audio,effects);if(!app)throw new Error('C++ 游戏初始化失败');
 ...
 const game={native,c,app,renderer,get frames(){return native.graphics.d3d.frames;},
  ...
  step({keys=new Set(),pads=[],focused=true,milliseconds=0}={}){
   ...
   const result=c.application_step(app),error=c.application_error(app);if(error)throw new Error('C++ 游戏运行失败：'+error);bridge?.sync();return result;
  },
