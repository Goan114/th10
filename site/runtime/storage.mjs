export async function openNativeStorage(language='jp'){
  ...
  return {load:()=>transaction('readonly',(store,done)=>{const entries=[];store.openCursor().onsuccess=event=>{const cursor=event.target.result;if(cursor){entries.push([cursor.key,cursor.value]);cursor.continue();}else done(entries);};}),save:(name,data)=>transaction('readwrite',store=>store.put(data,name)),remove:name=>transaction('readwrite',store=>store.delete(name))};
}
