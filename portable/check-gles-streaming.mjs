...
try {
 for(const [label,base] of [['previous',oldBase],['candidate',newBase]]) {
  ...
  await page.evaluate(async({game,replay})=>{
   ...
   const counters={replacements:0,subUpdates:0,draws:0,bytes:0},data=gl.bufferData,sub=gl.bufferSubData,draw=gl.drawArrays,instanced=gl.drawArraysInstanced;
  ...
  const result=await page.evaluate(()=>({counters:probe.counters,gpu:probe.gpu}));
 ...
 assert(runs.previous.counters.subUpdates>100,'Old build must exercise the original streaming upload');
 ...
 assert.equal(runs.candidate.counters.draws,runs.previous.counters.draws,'Upload change preserves batching and submission order');
 const report={passed:true,game,physicalPhone:false,runs};writeFileSync(out+'/gles-streaming.json',JSON.stringify(report,null,2));
...
} catch(error) {writeFileSync(out+'/gles-streaming-failure.json',JSON.stringify({error:error.stack,runs},null,2));throw error;}
