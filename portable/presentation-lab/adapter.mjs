...
function decodeRecords(buffer,pointer,count,stride){
  ...
  for(let index=0;index<count;index++){
    ...
    const continuous=meta[9],record={key:[meta[0],meta[1],meta[2],meta[3]].join(':'),ownerId:meta[0],ownerLabel:ownerLabel(meta[0]),objectId:meta[1],generation:meta[2],identityConfidence:meta[2]?'proven':'uncertain',partId:meta[3],drawId:meta[4],
      lifecycle:{continuous:true,scriptIndex:meta[5]|0,spriteIndex:meta[6]|0,fileIndex:meta[10],visible:meta[7]&3},coordinateSpace:'submitted-screen',bounds,geometryQuality:meta[8]>16?'sampled':'complete',
      fields:[
        ...
        {id:'submitted-position',label:'提交几何中心',value:center,tolerance:.025,interpolationPolicy:'continuous'},
        {id:'submitted-shape',label:'提交几何形状',value:shape,tolerance:.025,interpolationPolicy:'unknown'},
        {id:'submitted-uv',label:'提交 UV',value:submittedUv,tolerance:.00001,period:1,interpolationPolicy:'unknown'},
      ]};
    records.push(record);
  }
  return records;
...
export class Th10RuntimeDriver{
  ...
  describe(){return {driverApiVersion:DRIVER_API_VERSION,game:'th10',adapterVersion:'th10-presentation-lab/1',nativeAbi:TH10_PRESENTATION_LAB_ABI,features:{freeze:true,resume:true,step:true,drawOnly:true,references:true,stateEvidence:true,timing:true,capture:true,replay:false}};}
...
export class Th10ObservationAdapter{
  ...
  readStateEvidence(){return {coverageVersion:'th10/state/1',groups:[],missingGroups:['authoritative-game-state','renderer-and-anm-cache','application-cadence','audio','replay-cursor']};}
