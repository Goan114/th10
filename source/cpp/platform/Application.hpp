...
struct AppLoop final:ApplicationLoopEnvironment {
    ...
    i32 begin_scene(void*) override;void draw() override;i32 render_state(void*,u32,u32) override;
...
struct Application final:CallbackReceiver {
    ...
    ~Application();bool initialize();i32 step();void save();void shutdown();
    void advance_loading();void sync_views();bool ensure_world();void configure_camera(Camera&,bool);
    Extended time();bool invoke(CallbackToken,void*,i32&) override;
};
}
