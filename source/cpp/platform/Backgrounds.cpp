...
void BackgroundDraw::clear(u32 flags,u32 color,const StageClearRect* rectangle){owner.engine.device.call(DeviceOperation::Clear,{rectangle?1u:0u,static_cast<u32>(reinterpret_cast<uintptr_t>(rectangle)),flags,color,0x3f800000,0});}
...
Backgrounds::Backgrounds(GameState& s,AnimationEngine& e,ScreenEffects& fx,FileSystem& f):state(s),engine(e),effects(fx),files(f),script(*this){
    ...
    chain=&e.chain_value;callbacks=&e.callback_environment;update_callback=0x403050;background_callback=0x403060;foreground_callback=0x403070;e.register_receiver(*this);
}
