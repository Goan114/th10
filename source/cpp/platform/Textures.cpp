...
i32 Textures::create_surface_texture(AnmTexture& texture,u32 width,u32 height,u32 pixel_format){
    ...
    return device.call(DeviceOperation::CreateTexture,{width,height,1,0,pixel_format,1,address(&texture.handle),0});
}
...
void* Textures::get_surface(void* texture){void* surface=nullptr;device.resource(texture,ResourceOperation::Surface,{0,address(&surface)});return surface;}
TextureDescription Textures::describe_surface(void* surface){u32 values[8]{};device.resource(surface,ResourceOperation::Description,{address(values)});return {values[0],values[6],values[7]};}
TextureLock Textures::lock_surface(void* surface){TextureLock result{};device.resource(surface,ResourceOperation::LockSurface,{address(&result),0,0});return result;}
void Textures::unlock_surface(void* surface){device.resource(surface,ResourceOperation::UnlockSurface);}
void Textures::release_surface(void* surface){device.resource(surface,ResourceOperation::Release);}
