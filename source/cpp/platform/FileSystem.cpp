...
bool FileSystem::attach_archive(const char* name){return resources.open(name,false,archives);}
...
void FileSystem::read_archive(const char* name,u8* output){resources.read(name,output,archives);}
}
