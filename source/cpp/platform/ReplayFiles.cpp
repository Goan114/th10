...
i32 ReplayWriter::save(Replay& replay,const char* file,const char* name){const i32 result=save_replay(replay,file,name,*this);close_and_unlock();memory.clear();return result;}
...
u32 ReplayWriter::write(const void* bytes,u32 length){return files.host.write(output,static_cast<const u8*>(bytes),length);}
void ReplayWriter::close_and_unlock(){if(output!=0xffffffff){files.host.close(output);output=0xffffffff;}}
