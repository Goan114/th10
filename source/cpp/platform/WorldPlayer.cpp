...
struct Shooting final:PlayerShootingEnvironment {
    ...
    void initialize_shot(Player& player,PlayerShot& shot,i32) override{if(shot.definition->on_initialize==0x428ad0)player.initialize_homing_shot(shot);else __builtin_trap();}
    void update_shot(Player& player,PlayerShot& shot) override{if(shot.definition->on_update==0x428b10)player.update_homing_shot(shot);else if(shot.definition->on_update==0x428c20)player.update_option_laser(shot);else __builtin_trap();}
...
struct Profiles final:PlayerProfileEnvironment {
    World& w;explicit Profiles(World& world):w(world){static constexpr u32 initialize[]={0,0x428ad0,0},update[]={0,0x428b10,0x428c20,0},empty[]={0};callbacks={initialize,update,empty,empty};}
