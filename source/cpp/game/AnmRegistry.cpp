...
void AnmRegistry::set_position(u32 id,const Vec3& position,bool playfield_coordinates) const noexcept {
    with_children(find(id),[&](AnmVm& vm){
        vm.position.x=playfield_coordinates?(number(position.x)+number(224.0f)).to_float():position.x;
        vm.position.y=playfield_coordinates?(number(position.y)+number(16.0f)).to_float():position.y;
