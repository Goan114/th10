...
i32 Stage::update_script(StageEnvironment& environment){
    while(instruction->time<=script_timer.current){
        ...
        case 2:{const auto previous=camera.position;camera.position=vector(command,0);camera.animation_delta={(number(camera.position.x)-number(previous.x)).to_float(),(number(camera.position.y)-number(previous.y)).to_float(),(number(camera.position.z)-number(previous.z)).to_float()};break;}
