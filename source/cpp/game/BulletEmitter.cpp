...
i32 EnemyBulletManager::spawn(const BulletEmitter& e,i32 index,i32 layer,float aim,BulletBehaviorEnvironment& env){
    ...
    if(e.flags&0xe){
        ...
        const auto x=number(bullet->motion.velocity.x)*number(4.f),y=number(bullet->motion.velocity.y)*number(4.f);const float z=(number(bullet->motion.velocity.z)*number(4.f)).to_float();
        bullet->motion.position.x=(number(bullet->motion.position.x)-x).to_float();bullet->motion.position.y=(number(bullet->motion.position.y)-y).to_float();bullet->motion.position.z=(number(bullet->motion.position.z)-number(z)).to_float();
