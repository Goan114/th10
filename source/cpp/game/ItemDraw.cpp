...
void Item::draw(ItemDrawEnvironment& env){
    if(!state)return;
    auto& vm=animation;vm.script_position.x=(number(position.x)+number(224.0f)).to_float();
    vm.script_position.y=(number(position.y)+number(16.0f)).to_float();vm.script_position.z=position.z;
