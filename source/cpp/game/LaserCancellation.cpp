...
struct Sweep {
    ...
    explicit Sweep(const EnemyLaser& laser,bool preserve_z):original(laser.position){
        ...
        point={(number(half.x)+number(original.x)).to_float(),(number(half.y)+number(original.y)).to_float(),preserve_z?original.z:0.f};
        step={(number(half.x)+number(half.x)).to_float(),(number(half.y)+number(half.y)).to_float(),0};
    }
    void next(){point.x=(number(step.x)+number(point.x)).to_float();point.y=(number(step.y)+number(point.y)).to_float();point.z=(number(point.z)+number(step.z)).to_float();distance=(number(distance)+number(12.f)).to_float();}
    Vec3 position(i32 index,const Vec3& origin) const {
        ...
        return {(x+number(origin.x)).to_float(),(number(y)+number(origin.y)).to_float(),(number(z)+number(origin.z)).to_float()};
    }
};
struct Rectangle {
    Vec2 minimum,maximum;
    Rectangle(const Vec3& point,const Vec3& size){
        const float x=(number(size.x)*number(.5f)).to_float(),y=(number(size.y)*number(.5f)).to_float();
        minimum={(number(point.x)-number(x)).to_float(),(number(point.y)-number(y)).to_float()};
        maximum={(number(x)+number(point.x)).to_float(),(number(y)+number(point.y)).to_float()};
    }
...
struct Circle {
    ...
    Circle(const Vec3& center,float radius):center(center),squared_radius((number(radius)*number(radius)).to_float()){}
...
i32 EnemyLaser::intersects(const Vec3& point,float margin) const noexcept {
    const float x=(number(point.x)-number(position.x)).to_float(),y=(number(point.y)-number(position.y)).to_float();
    ...
    const float top=(rotated_y-number(margin)).to_float(),right=(number(rotated_x)+number(margin)).to_float(),bottom=(rotated_y+number(margin)).to_float();
