...
void EnemyBullet::update_feature(BulletFeature feature,BulletFrameEnvironment& env){
    if(feature==BulletFeature::SpawnAcceleration){
        ...
        if(m.timer.current>=m.duration)active_features&=~0x10u;
            ...
            const auto x=number(*env.default_rate)*number(m.vector.x),y=number(*env.default_rate)*number(m.vector.y);const float z=(number(*env.default_rate)*number(m.vector.z)).to_float();
            motion.velocity.x=(x+number(motion.velocity.x)).to_float();motion.velocity.y=(y+number(motion.velocity.y)).to_float();motion.velocity.z=(number(z)+number(motion.velocity.z)).to_float();
        ...
        else{motion.angle=add_angle(motion.angle,(number(*env.default_rate)*number(m.second)).to_float()).to_float();motion.speed=(number(*env.default_rate)*number(m.first)+number(motion.speed)).to_float();velocity(motion,motion.speed);}m.timer.tick();
        ...
        if(m.timer.current>=m.duration){
            ...
            if(feature==BulletFeature::Turn)motion.angle=(number(m.second)+number(motion.angle)).to_float();
        ...
        if(motion.position.x< -192.f||motion.position.x>=192.f){
            ...
            motion.position.x=(number(motion.position.x< -192.f?-384.f:384.f)-number(motion.position.x)).to_float();
        }
