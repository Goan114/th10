...
i32 Gui::draw(GuiDrawEnvironment& env){
    ...
    if(env.game->faith_timer.current){
        ...
        rect.right=(number(rect.right)-number(1)).to_float();rect.left=50;rect.top=460;rect.bottom=462;env.rectangle(rect,(faith_digits[0].color&0xff000000)|0xffffff);
    }
    if(displayed_boss_health>0){
        ...
        rect.right=(number(rect.right)-number(1)).to_float();rect.left=40;rect.top=22;rect.bottom=24;env.rectangle(rect,0xffffffff);
