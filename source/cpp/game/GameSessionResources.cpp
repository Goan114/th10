...
i32 GameSessionResources::load(){auto& env=environment;auto& game=*env.game;session.session_flags|=4;
    while(*env.drawing_resource>=0||*env.updating_resource>=0){if(*env.engine_flags&0x80)return fail();env.sleep(1);}
    *env.rate=1;game.stage_frames=game.section_frames=0;
    ...
    if(!(game.flags&2)){
        for(auto kind:{SessionObject::Replay,SessionObject::Stage,SessionObject::Gui,SessionObject::Player,SessionObject::Bullets,SessionObject::Items,SessionObject::Lasers,SessionObject::Results,SessionObject::TextOverlay,SessionObject::ScorePopups})if(!env.create_object(kind,session.replay_mode))return fail();
    }else {env.prepare_replay();env.reload_gui();if(!env.create_object(SessionObject::Stage,session.replay_mode))return fail();}
    if(!(game.flags&9)){if(!env.create_object(SessionObject::Enemies,session.replay_mode))return fail();}
    else reset_enemy_stage(*reinterpret_cast<EnemyManager*>(env.object(SessionObject::Enemies)),env.rate);
    for(auto kind:{SessionObject::Effects,SessionObject::Bomb,SessionObject::Spell})if(!env.create_object(kind,session.replay_mode))return fail();
    if(!(game.flags&0x20)){env.stop_music();env.load_music(0,reinterpret_cast<const char*>((*env.current_stage)->resources[4]));env.load_music(1,reinterpret_cast<const char*>((*env.current_stage)->resources[5]));}
    *env.rendered_time=0;*env.recorded_time=0;session.reset_timer(env.rate);
    while(*env.pending_upload)env.sleep(16);
