...
namespace th10 {
namespace {
float pixels(i32 hundredths){return (Extended::from_int(hundredths)*number(.01f)).to_float();}
...
i32 Player::move(PlayerMovementEnvironment& env){
    ...
    input_velocity={x,y};velocity={(Extended::from_int(x)*number(*env.default_rate)).truncate_int(),(Extended::from_int(y)*number(*env.default_rate)).truncate_int()};
