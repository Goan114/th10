...
bool EnemyState::animation_command(EclContext& context,EclGlobals& globals,EnemyAnimationEnvironment& env){
    ...
    const auto configure=[&](i32 slot){
        ...
        if(slot==0){visual_size.x=(number(vm->sprite_size.y)*number(vm->scale.y)).to_float();visual_size.y=(number(vm->sprite_size.x)*number(vm->scale.x)).to_float();}
    ...
    case 0x107:case 0x108:case 0x110:case 0x111:{
        ...
        if(!(flags&0x40000)){vm->position.x=(number(current.position.x)+number(224.f)).to_float();vm->position.y=(number(current.position.y)+number(16.f)).to_float();}
