...
bool EnemyState::spawn_command(EclContext& context,EclGlobals& globals,EnemySpawnEnvironment& env){
    ...
    if(opcode==0x10e){
        ...
        if(opcode==0x100&&(flags&0x40000)){
            ...
            parameters.position.x=(number(parameters.position.x)-number(224.f)).to_float();parameters.position.y=(number(parameters.position.y)-number(16.f)).to_float();parameters.position.z=0;
        }
