...
u32 AnmRenderer::axis_geometry(const AnmVm& vm,AnmVertex* q,bool pixel) noexcept {
    ...
    case 0:{auto left=sum(vm.child_position.x,vm.position.x,vm.script_position.x)-width*number(0.5f);if(pixel)left=Extended::from_double(std::floor(left.to_double()));
        q[0].position.x=q[2].position.x=left.to_float();q[1].position.x=q[3].position.x=(number(q[0].position.x)+width).to_float();break;}
    case 1:q[0].position.x=q[2].position.x=sum(vm.script_position.x,vm.position.x,vm.child_position.x).to_float();q[1].position.x=q[3].position.x=(number(vm.script_position.x)+number(vm.position.x)+width+number(vm.child_position.x)).to_float();break;
    case 2:q[0].position.x=q[2].position.x=(sum(vm.script_position.x,vm.position.x,vm.child_position.x)-width).to_float();q[1].position.x=q[3].position.x=sum(vm.script_position.x,vm.position.x,vm.child_position.x).to_float();break;
    }
    switch((vm.flags>>20)&3){
    case 0:{auto top=sum(vm.child_position.y,vm.position.y,vm.script_position.y)-half_height;if(pixel)top=Extended::from_double(std::floor(top.to_double()));q[0].position.y=q[1].position.y=top.to_float();q[2].position.y=q[3].position.y=(number(q[0].position.y)+height).to_float();break;}
    case 1:if(pixel){const auto top=sum(vm.position.y,vm.child_position.y,vm.script_position.y);q[0].position.y=q[1].position.y=top.to_float();q[2].position.y=q[3].position.y=(top+height).to_float();}
        else{q[0].position.y=q[1].position.y=sum(vm.child_position.y,vm.script_position.y,vm.position.y).to_float();q[2].position.y=q[3].position.y=(number(vm.child_position.y)+number(vm.script_position.y)+height+number(vm.position.y)).to_float();}break;
    case 2:{const auto bottom=pixel?sum(vm.position.y,vm.child_position.y,vm.script_position.y):sum(vm.child_position.y,vm.script_position.y,vm.position.y);q[0].position.y=q[1].position.y=(bottom-height).to_float();q[2].position.y=q[3].position.y=bottom.to_float();break;}
    }
    // The non-pixel original reads child Y here, including when rotation is zero.
    depth(q,sum(pixel?vm.child_position.z:vm.child_position.y,vm.position.z,vm.script_position.z));return pixel?1:0;
}
...
u32 AnmRenderer::rotated_geometry(const AnmVm& vm,AnmVertex* q) noexcept {
    ...
    const auto c=number(cosine(number(vm.rotation.z)).to_float()),s=number(sine(number(vm.rotation.z)).to_float());
    const auto x=sum(vm.child_position.x,vm.position.x,vm.script_position.x),y=sum(vm.child_position.y,vm.position.y,vm.script_position.y);
    const auto width=number(vm.sprite_size.x)*number(vm.scale.x),height=number((number(vm.sprite_size.y)*number(vm.scale.y)).to_float());
    ...
    depth(q,sum(vm.child_position.z,vm.position.z,vm.script_position.z));return 0;
}
...
void AnmRenderer::flush(){if(!manager.batch_quads)return;environment.texture_stage(0,6,0);environment.texture_stage(0,3,0);environment.vertex_format(0x144);environment.draw_triangles(4,manager.batch_quads*2,manager.batch_start,28);manager.batch_start=manager.vertex_write;manager.batch_quads=0;++manager.flushed_batches;}
// 0x4425a0. Read VM flags again after a flush, preserving callback side effects.
void AnmRenderer::apply_state(const AnmVm& vm){
    if(manager.cached_draw_state[0]!=((vm.flags>>4)&3)){flush();const auto blend=(vm.flags>>4)&3;manager.cached_draw_state[0]=blend;if(blend<2)environment.render_state(20,blend?2:6);}
    if(manager.cached_draw_state[6]!=(vm.flags>>31)){flush();const auto point=vm.flags>>31;manager.cached_draw_state[6]=point;environment.sampler_state(0,5,point?1:2);environment.sampler_state(0,6,point?1:2);}++manager.submitted_draws;
}
...
void AnmRenderer::apply_material(const AnmVm& vm){
    if(manager.cached_draw_state[0]!=((vm.flags>>4)&3)){flush();const auto blend=(vm.flags>>4)&3;manager.cached_draw_state[0]=blend;if(blend<3)environment.render_state(20,blend?2:6);}
    ...
    if(manager.current_material_color!=color){flush();manager.current_material_color=color;environment.render_state(60,color);}
    if(manager.cached_draw_state[6]!=(vm.flags>>31)){flush();const auto point=vm.flags>>31;manager.cached_draw_state[6]=point;environment.sampler_state(0,5,point?1:2);environment.sampler_state(0,6,point?1:2);}++manager.submitted_draws;
}
...
i32 AnmRenderer::submit(const AnmVm& vm,u32 flags,bool flip_u){
    auto* q=environment.quad;for(u32 i=0;i<4;++i){q[i].position.x=(number(q[i].position.x)+number(manager.draw_offset.x)).to_float();q[i].position.y=(number(q[i].position.y)+number(manager.draw_offset.y)).to_float();}
    ...
    q[0].uv.x=q[2].uv.x=(number(flip_u?vm.sprite->u1:vm.sprite->u0)+number(vm.uv_offset.x)).to_float();q[1].uv.x=q[3].uv.x=(number(flip_u?vm.sprite->u0:vm.sprite->u1)+number(vm.uv_offset.x)).to_float();q[0].uv.y=q[1].uv.y=(number(vm.sprite->v0)+number(vm.uv_offset.y)).to_float();q[2].uv.y=q[3].uv.y=(number(vm.sprite->v1)+number(vm.uv_offset.y)).to_float();
