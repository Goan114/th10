...
i32 StageHints::display(ListNode<StageHint>* head,bool release,StageHintsEnvironment& env){
    while(head){auto* next=head->next;auto& hint=*head->value;if(!hint.section||hint.section==env.game->section)hint.delay=wrapping_add(hint.delay,-1);if(hint.delay<=0){
        ...
        if(small){vm.text_settings[0]=vm.text_settings[1]=15;scale_animation(vm,{hint.scale,hint.scale},{(number(hint.scale)*number(1.5f)).to_float(),0},env.rate);}
        else{vm.text_settings[0]=vm.text_settings[1]=medium?static_cast<u8>((number(hint.scale)*number(15)).truncate_int()):30;const float half=(number(hint.scale)*number(.5f)).to_float();scale_animation(vm,{half,half},{(number(hint.scale)*number(.75f)).to_float(),0},env.rate);}
...
i32 StageHints::update(StageHintsEnvironment& env){
    ...
    for(u32 i=0;i<10;++i){auto* vm=env.registry->find_and_clear(animations[i]);if(!vm||vm->alpha_interpolation.duration)continue;
        const auto x=absolute(player_x-number(positions[i].x));const float y=absolute(player_y-number(positions[i].y)).to_float(),half=(number(widths[i])*number(.5f)).to_float();
        ...
        else{const float outer=(number(half)+number(32)).to_float();const auto faded=Extended::from_int(wrapping_add(alpha,wrapping_add(alpha,alpha))/4);
...
i32 StageHints::save(const char* name,StageHintsEnvironment& env){
    auto* buffer=static_cast<char*>(env.allocate_bytes(4096));env.make_directory("hint");if(env.begin_file(name)){
        ...
        for(i32 stage=0;stage<8;++stage){i32 count=0;for(auto* node=retained[stage].next;node;){auto* next=node->next;const auto& hint=*node->value;if(hint.remaining){
                ...
                hint_format(buffer,4096,"\tText\t: \"%s\"\r\n",hint.text);emit(false);const i32 y=number(hint.position.y).truncate_int(),x=number(hint.position.x).truncate_int();hint_format(buffer,4096,"\tPos\t\t: %d, %d\r\n",x,y);emit(false);
