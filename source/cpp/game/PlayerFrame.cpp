...
i32 Player::update(PlayerFrameEnvironment& env){
    ...
    if(state==0){
        ...
        if(state_timer.current<30){
            ...
            env.cancel_bullet_circle(death_position,radius,true);env.cancel_bullet_circle(death_position,(number(radius)*number(.25f)).to_float(),false);
    ...
    if(state==2){
        if(state_timer.current==3){
            ...
            const Vec3 center{0,(number(position.y)-number(224.f)).to_float(),0};const float base=angle_towards(center).to_float();
    ...
    for(auto& area:damage_areas)if(area.flags&1){
        area.motion.update_velocity();area.motion.update();area.radius=(number(area.radial_speed)+number(area.radius)).to_float();
        area.angle=(number(area.angular_velocity)+number(area.angle)).to_float();area.timer.advance(-1.f);if(area.timer.current<=0)area.flags&=~1u;
    }
    ...
    set_bounds(pickup_bounds,position,number(fast_pickup_size.x)*number(.5f),number(fast_pickup_size.y)*number(.5f),number((number(fast_pickup_size.z)*number(.5f)).to_float()));
