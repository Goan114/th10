...
i32 UpdateChain::run(bool drawing,UpdateChainEnvironment& env){
    ...
    while(next){auto* entry=next->value;next=next->next;if(!entry->callback)continue;
        bool restart=false;
        while(entry->flags&2){
            env.unlock();const i32 result=env.invoke(entry->callback,entry->owner);env.lock();
            ...
            else if(!drawing&&result==7&&entry->notify_callback)env.invoke(entry->notify_callback,entry->owner);
            break;
        }
