...
export function validateRuntimeReleaseManifest(manifest) {
  ...
  for (const game of registered) {
    ...
    for (const [variant, runtime] of variants) {
      ...
      const expectedNames = [`${stem}.html`, `${stem}.js`, `${stem}.wasm`];
      if (!runtime.files || JSON.stringify(Object.keys(runtime.files)) !== JSON.stringify(expectedNames)) {
        throw new Error(`${game} ${variant}: Runtime file set must be HTML/JS/WASM only`);
      }
