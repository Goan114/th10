export interface Sha256DigestProvider {
...
export async function sha256Hex(
  ...
  digestProvider: Sha256DigestProvider | null = globalThis.crypto?.subtle ?? null,
): Promise<string> {
  const input = bytes instanceof Uint8Array ? bytes : new Uint8Array(bytes);
  if (typeof digestProvider?.digest === "function") {
    const digestInput = Uint8Array.from(input).buffer;
    const digest = await digestProvider.digest("SHA-256", digestInput);
